...
LOG_FILE="${HOME}/agent-nightshift.log"
MARKER="agent-nightshift:${ROOT_DIR}"
LEGACY_MARKER="claude-night-worker:${ROOT_DIR}"
TEMP_FILE="$(mktemp)"
...
crontab -l 2>/dev/null | grep -Fv -e "$MARKER" -e "$LEGACY_MARKER" > "$TEMP_FILE" || true
